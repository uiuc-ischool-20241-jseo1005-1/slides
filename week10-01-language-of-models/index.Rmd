...
- When looking to summarize the relationship between a predictor $x$ and a response $y$, we are interested in knowing the population parameters: $\beta_0$  for the intercept and $\beta_1$  for the slope in the following model:

$${y}_{i} = \beta_0 + \beta_1~x_{i}$$

--
- The only way we could ever know it, though, is to be able to collect data on everybody in the population — most often an impossible task. We have to rely on taking and using a sample of data from the population to estimate the population parameters.
...
When building a machine learning model, some questions similar to this usually comes into mind: 

- How good a prediction model does in terms of being able to predict the expected outcome?

- We would like the predictions to be close to the expected values. But how do we formalize this preference?
- We would like the prediction error ( $e_i = y_i - \hat{y}_i$) over all data points to be as small as possible. We estimate parameters using least squares i.e. minimizing
$\sum_{i = 1}^n e_i^2$.

---
## Least Squares Estimation

- Find ( $\beta_0,\beta_1$) that minimize the residual sum of squares (RSS) 
...
--
 ...
 To find the solution, we can take the derivative w.r.t. to $\hat{\beta_0}$ and $\hat{\beta_1}$ and equate it to zero.
 
--
 
$$\frac{\partial RSS}{\partial\hat{\beta_0}} = -2 \sum_{i=1}^n (y_i - \hat{\beta_0} - \hat{\beta_1}x_i) = 0$$
$$\frac{\partial RSS}{\partial\hat{\beta_1}} = -2 \sum_{i=1}^n x_i (y_i - \hat{\beta_0} - \hat{\beta_1}x_i) = 0$$
---

## Least Squares Estimation

- On solving the equations, we get the least squares estimates of $\beta_0$ and $\beta_1$ as follows:
...
```{r echo=FALSE, out.width="35%", fig.alt = "Scatterplot showing 2 regression lines: the best fit line and a (not best fit) line."}
knitr::include_graphics("img/best_fit_line_vs_other.png")
```
